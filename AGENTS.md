...
本仓库是 `QuantumNous/new-api` 的 AGPL-3.0 修改版。文件下方由上游带入的英文约定继续适用于上游代码，不得删除其中的版权、署名与保护条款；燕中自主模块遵循工作区约定。燕中 API 的职责是模型控制面：渠道、额度、Key、流水与审计，主站是唯一身份源，不可变流水是账本真值。

生产相关：`api.yanchuaner.cn` 与主站数据库隔离，改动渠道、额度或策略前先备份对应数据并说明回滚方式；嵌入与语音等媒体渠道必须按 token 计费，不能按次计费导致预算失真。
